...
public class VectorStore {
    ...
    public boolean hasProfileEmbedding(UUID userId) {
        Integer count =
                jdbc.queryForObject("SELECT count(*) FROM profile_embedding WHERE user_id = ?", Integer.class, userId);
    ...
    public List<SimilarJob> similarJobsForUser(UUID userId, int limit) {
        return jdbc.query(
                ...
                LIMIT ?
                """,
                (rs, rowNum) -> new SimilarJob(rs.getObject("job_id", UUID.class), rs.getDouble("similarity")),
                userId,
                limit);
    }
