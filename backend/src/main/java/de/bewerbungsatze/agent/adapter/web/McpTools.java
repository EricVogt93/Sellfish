package de.bewerbungsatze.agent.adapter.web;

import de.bewerbungsatze.common.security.CurrentUser;
import de.bewerbungsatze.generate.GenerationDtos.GeneratedResponse;
import de.bewerbungsatze.generate.GenerationService;
import de.bewerbungsatze.generate.GenerationType;
import de.bewerbungsatze.jobs.JobSearchService;
import de.bewerbungsatze.jobs.SearchRun;
import de.bewerbungsatze.matching.MatchDtos.MatchResponse;
import de.bewerbungsatze.matching.MatchService;
import de.bewerbungsatze.matching.MatchStatus;
import de.bewerbungsatze.agent.mcp.McpTool;
import de.bewerbungsatze.agent.mcp.McpToolParam;
...
public class McpTools {
    ...
    public McpTools(JobSearchService jobSearchService,
    ...
    @McpTool(description = "Löst einen Suchlauf über alle Job-Quellen aus und liefert die besten Matches zurück")
    ...
    public List<McpMatch> listMatches(
            @McpToolParam(description = "Match-Status zum Filtern (NEW, SEEN, SAVED, DISMISSED, APPLIED, INTERVIEW, OFFER, REJECTED)")
            String status,
            @McpToolParam(description = "Maximale Anzahl Matches (1-100, default 20)")
            int limit) {
        UUID userId = CurrentUser.id();
        MatchStatus ms = status != null && !status.isBlank()
        ...
        return matchService.list(userId, ms, PageRequest.of(0, clamped)).getContent().stream()
                .map(m -> new McpMatch(m.matchId(), m.title(), m.company(), m.location(),
                        m.url(), m.score(), m.status().name(), m.source()))
                .toList();
    }

    @McpTool(description = "Generiert ein Bewerbungsdokument für ein Job-Match (Anschreiben, Motivation, CV, Kurztext)")
    public String generateDocument(
            @McpToolParam(description = "UUID des Job-Matches")
            String jobMatchId,
        ...
        GeneratedResponse result = GeneratedResponse.from(
                generationService.generate(CurrentUser.id(), matchId, genType));
    ...
    public record McpMatch(UUID matchId, String title, String company, String location,
                           String url, double score, String status, String source) {}
}
