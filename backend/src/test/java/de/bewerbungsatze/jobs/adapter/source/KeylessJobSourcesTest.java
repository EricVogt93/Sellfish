...
class KeylessJobSourcesTest {
    ...
    void arbeitnowParsesAndFilters() {
        Bound b = bind();
        b.server().expect(requestTo(Matchers.containsString("/job-board-api")))
    ...
    void remotiveParsesJobs() {
        Bound b = bind();
        b.server().expect(requestTo(Matchers.containsString("/remote-jobs")))
                .andRespond(withSuccess("""
                        {"jobs":[
    ...
    void remoteOkSkipsLegalAndFilters() {
        Bound b = bind();
        b.server().expect(requestTo(Matchers.containsString("/api")))
                .andRespond(withSuccess("""
                        [
    ...
    void himalayasParsesAndFilters() {
        Bound b = bind();
        b.server().expect(requestTo(Matchers.containsString("/jobs/api")))
                .andRespond(withSuccess("""
                        {"jobs":[
