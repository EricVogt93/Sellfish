...
class MoreJobSourcesTest {
    ...
    void workingNomadsParsesAndFilters() {
        Bound b = bind();
        b.server()
                .expect(requestTo(Matchers.containsString("/api/exposed_jobs/")))
    ...
    void ashbyPerOrg() {
        Bound b = bind();
        b.server()
                .expect(requestTo(Matchers.containsString("/job-board/acme")))
                .andRespond(withSuccess(
                        """
                        {"jobs":[
    ...
    void recruiteePerCompany() {
        Bound b = bind();
        b.server()
                .expect(requestTo(Matchers.containsString("recruitee.com/api/offers/")))
                .andRespond(withSuccess(
                        """
                        {"offers":[
    ...
    void smartRecruitersPerCompany() {
        Bound b = bind();
        b.server()
                .expect(requestTo(Matchers.containsString("/companies/acme/postings")))
                .andRespond(withSuccess(
                        """
                        {"content":[
    ...
    void atsSourcesSkippedWithoutConfig() {
        ...
        assertThat(new SmartRecruitersSource(b.builder()).fetch(query, Map.of()))
                .isEmpty();
    ...
    void weWorkRemotelyParsesAndFilters() {
        Bound b = bind();
        b.server()
                .expect(requestTo(Matchers.containsString("/api/jobs")))
                .andRespond(withSuccess(
                        """
                        [
    ...
    void noDeskParsesAndFilters() {
        Bound b = bind();
        b.server()
                .expect(requestTo(Matchers.containsString("/remote-jobs.json")))
                .andRespond(withSuccess(
                        """
                        [
    ...
    void workablePerCompany() {
        Bound b = bind();
        b.server()
                .expect(requestTo(Matchers.containsString("/accounts/acme/jobs")))
                .andRespond(withSuccess(
                        """
                        {"jobs":[
