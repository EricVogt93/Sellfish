<script lang="ts">
	...
	let teamStats = $state<
		{ userId: string; email: string; applied: number; interviews: number; generated: number }[]
	>([])
...
<div class="rp-page">
	...
	{:else}
		<div class="rp-cards">
			<div class="rp-card">
				<span class="rp-card-num">{summary.totalMatches}</span><span class="rp-card-label"
					>Matches</span
				>
			</div>
			<div class="rp-card">
				<span class="rp-card-num">{summary.applied}</span><span class="rp-card-label">Applied</span>
			</div>
			<div class="rp-card">
				<span class="rp-card-num">{summary.interviews}</span><span class="rp-card-label"
					>Interviews</span
				>
			</div>
			<div class="rp-card">
				<span class="rp-card-num">{summary.generated}</span><span class="rp-card-label"
					>Generated</span
				>
			</div>
			<div class="rp-card">
				<span class="rp-card-num">{summary.jobsScanned}</span><span class="rp-card-label"
					>Jobs Scanned</span
				>
			</div>
			<div class="rp-card">
				<span class="rp-card-num">{summary.totalUsers}</span><span class="rp-card-label">Users</span
				>
			</div>
		</div>
			...
			<div class="rp-chart-box">
				...
				<div class="rp-status-bar">
					{#each Object.entries(statusDist) as [status, count]}
						<div
							class="rp-status-seg"
							style="flex:{count};background:{statusColors[status] || '#94a3b8'};"
							title="{status}: {count}"
						></div>
					{/each}
				</div>
				<div class="rp-status-legend">
					{#each Object.entries(statusDist) as [status, count]}
						<span class="rp-legend-item">
							<span class="rp-legend-dot" style="background:{statusColors[status] || '#94a3b8'};"
							></span>
							{status}: {count}
						</span>
					{/each}
		...
		{#if salaryStats && salaryStats.count > 0}
			<div class="rp-chart-box">
				...
				<div class="rp-cards" style="margin-bottom:0;">
					<div class="rp-card">
						<span class="rp-card-num">{fmtEur(salaryStats.p25)}</span><span class="rp-card-label"
							>25th percentile</span
						>
					</div>
					<div class="rp-card">
						<span class="rp-card-num">{fmtEur(salaryStats.p50)}</span><span class="rp-card-label"
							>Median</span
						>
					</div>
					<div class="rp-card">
						<span class="rp-card-num">{fmtEur(salaryStats.p75)}</span><span class="rp-card-label"
							>75th percentile</span
						>
					</div>
					<div class="rp-card">
						<span class="rp-card-num">{fmtEur(salaryStats.avg)}</span><span class="rp-card-label"
							>Average</span
						>
					</div>
				</div>
