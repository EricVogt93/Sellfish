...
<div class="aa-view">
	<header class="aa-pagehead">
		...
		<div class="aa-pagehead-right">
			<span class="aa-hint"
				><Kbd>J</Kbd><Kbd>K</Kbd> navigate · <Kbd>X</Kbd> select · <Kbd>Enter</Kbd> open · <Kbd
					>A</Kbd
				> apply</span
			>
			<Btn variant="secondary" icon="refresh" disabled={searching} onclick={onRescan}
				>{searching ? 'Scanning…' : 'Rescan'}</Btn
			>
		</div>
	</header>
	...
	<div class="aa-tablecard">
		...
		{#if jobs.length === 0}
			<div class="aa-empty">
				...
				<p>
					No matches yet. Set your preferences &amp; an AI provider in Profile, then run a scan.
				</p>
				<Btn variant="primary" icon="refresh" disabled={searching} onclick={onRescan}
					>{searching ? 'Scanning…' : 'Run search'}</Btn
				>
			</div>
		{/if}
	</div>
	...
	{#if selCount > 0}
		<div class="aa-bulkbar">
			...
			<Btn variant="primary" icon="zap" onclick={onBulkApply}
				>Apply to {selCount} job{selCount > 1 ? 's' : ''}</Btn
			>
