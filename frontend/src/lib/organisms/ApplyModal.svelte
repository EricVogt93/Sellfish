	...
	<div class={`aa-modal ${step === 'review' ? 'aa-modal-wide' : ''}`}>
		{#if step === 'generating'}
			<div class="aa-gen">
				...
				<h3 class="aa-gen-title">
					Generating {jobs.length > 1 ? `${jobs.length} applications` : 'application'}
				</h3>
				<div class="aa-gen-sub">{jobs.map((j) => j.company).join(' · ')}</div>
				<div class="aa-gen-lines">
					{#each GEN_STEPS as s, i (s)}
						<div
							class={`aa-gen-line ${i < genLine ? 'is-done' : i === genLine ? 'is-active' : ''}`}
						>
							{#if i < genLine}
								<Icon
									name="check"
		...
		{:else if step === 'error'}
			<div class="aa-gen">
				<div
					class="aa-gen-orb"
					style="background:linear-gradient(135deg,var(--accent-error),var(--accent-tertiary));animation:none;"
				>
			...
			<div class="aa-review">
				<header class="aa-review-head">
					<div>
						...
						<h3 style="margin:2px 0 0;font-size:1.15rem;">
							{jobs.length > 1 ? `${jobs.length} applications` : job.title}
						</h3>
					</div>
					<button class="aa-iconbtn" onclick={onClose} title="Close"
				...
				{#if jobs.length > 1}
					<div class="aa-review-tabs">
						{#each jobs as j, i (j.id)}
							<button
								class={`aa-review-tab ${i === active ? 'is-active' : ''}`}
								onclick={() => (active = i)}
							>
					...
					<aside class="aa-review-side">
						<div class="aa-review-block">
							...
							<div class="aa-segmented">
								{#each ['COVER_LETTER', 'MOTIVATION', 'APPLICATION_TEXT'] as t (t)}
									<button
										class={`aa-segbtn ${docType === t ? 'is-active' : ''}`}
										onclick={() => regenerate(t as GenerationType)}
									>
						...
						<div class="aa-review-block">
							...
							{#if documents.length === 0}
								<p class="aa-cardnote" style="margin-top:0;">
									No documents yet — upload your CV under Profile.
								</p>
							{/if}
						...
						<div class="aa-review-block">
							...
							<div class="aa-review-target">
								<div class="aa-cond">
									<Icon name="mail" size={13} /><span>via {job.source}</span>
								</div>
								<div class="aa-cond">
									<Icon name="briefcase" size={13} /><span>{job.title}</span>
								</div>
							</div>
						</div>
					</aside>
		...
		{:else if step === 'done'}
			<div class="aa-gen">
				...
				<h3 class="aa-gen-title">
					{jobs.length > 1 ? `${jobs.length} applications sent` : 'Application sent'}
				</h3>
