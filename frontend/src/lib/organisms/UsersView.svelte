<script lang="ts">
	...
	onMount(async () => {
		try {
			...
			isAdmin = false
			if (me)
				members = [{ id: me.id, email: me.email, role: me.role, status: 'ACTIVE', createdAt: '' }]
		}
	})
</script>
...
<div class="aa-view">
	<header class="aa-pagehead">
		<div>
			<div class="eyebrow">
				workspace · {members.length} member{members.length === 1 ? '' : 's'}
			</div>
					...
					<tr class="aa-row">
						<td>
							<div class="aa-jobcell">
								...
								<div class="aa-jobcell-text">
									<span class="aa-jobtitle"
										>{u.email}
										{#if u.id === me?.id}<span class="aa-youtag">you</span>{/if}</span
									>
						...
						<td>
							<span class="aa-stagecount">
								<span
									class="aa-stage-dot"
									style={`background:${u.status === 'ACTIVE' ? 'var(--accent-success)' : 'var(--text-muted)'};`}
								></span>
	...
	{#if !isAdmin}
		<p class="aa-cardnote" style="margin-top:12px;">
			You see only your own account. Workspace member management requires an admin role.
		</p>
	{:else}
		<p class="aa-cardnote" style="margin-top:12px;">
			Each member has their own profile, filters, documents and application history.
		</p>
	{/if}
</div>
