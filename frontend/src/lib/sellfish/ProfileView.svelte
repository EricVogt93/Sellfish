<script lang="ts">
	...
	async function smartImport(e: Event) {
		...
		try {
			...
			documents = await backend.listDocuments()
			// Reload prefs (they were auto-applied server-side)
			prefs = await backend.getPreferences()
			...
			<section class="aa-card">
				...
				<div class="aa-smart-dropzone">
					...
					<label class="aa-dropzone aa-dropzone-smart" style="flex:1;cursor:pointer;">
						<span
							><strong>AI Smart Import</strong> — upload a merged PDF (CV + certificates + references).
							The AI will OCR, classify, split and upload each document automatically.</span
						>
						<input
							type="file"
